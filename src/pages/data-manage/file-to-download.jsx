	...
	switch (type) {
		...
		case "dbFolder": {
			return {
				...prestate,
				fileInfo: [],
				pathInfo: [...prestate.pathInfo, ">" + data.name],
			};
		}
		case "folderSplice": {
...
const FileToDownload = function (props) {
    ...
    const getFilesCallback=props.getFilesCallback;
	useEffect(() => {
		getFilesCallback("root");
	}, [getFilesCallback]);
	useEffect(() => {
	...
	useEffect(() => {
		getFilesCallback(getPath([...stateInfo.pathInfo]));
	}, [stateInfo.pathInfo,getFilesCallback]);
			...
			<div className='file_list'>
				<div className='file_path'>
					{stateInfo.pathInfo.map((path, index) => {
						return (
							<span key={index} onClick={(e) => pathClick(path, index)}>
								{path}
							</span>
						);
					})}
				...
				<div className='file_content'>
					{stateInfo.fileInfo.map((fileInfo, index) => {
						...
						return (
							...
							>
								<div
									className='single_file_container'
									onDoubleClick={(e) => fileDbClick(fileInfo, e)}
								>
									<img src={img} alt='' />
