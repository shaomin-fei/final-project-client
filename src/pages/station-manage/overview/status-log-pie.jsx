...
export default class StatusLogPie extends Component {
	initData = [
		...
		},
		{
			value: 100,
			name: "Working",
			itemStyle: { color: "rgba(53, 255, 0, 1)" },
		},
		{
			value: 100,
			name: "Idle",
			itemStyle: { color: "rgba(0,254,228,1)" },
		},
		{
			value: 100,
			name: "Shutdown",
			itemStyle: { color: "rgba(180,180,180,1)" },
		},
	];
	...
	render() {
		if (this.logData !== this.props.logData) {
			...
			if (tempData) {
				showData.push({
					value: tempData.warningTime,
					name: "Warning",
					itemStyle: { color: "rgba(255,0,0,1)" },
				});
				showData.push({
					value: tempData.workingTime,
					name: "Working",
					itemStyle: { color: "rgba(53, 255, 0, 1)" },
				});
				showData.push({
					value: tempData.idleTime,
					name: "Idle",
					itemStyle: { color: "rgba(0,254,228,1)" },
				});
				showData.push({
					value: tempData.shutdownTime,
					name: "Shutdown",
					itemStyle: { color: "rgba(180,180,180,1)" },
				});
			}
		...
		return (
			<div
				className='status_static_pie_container'
				ref={(dv) => (this.chartConteiner = dv)}
			></div>
		);
	}
}
