...
const RealtimeStationInfoBox = function (props) {
	...
	function getDeviceAbilityJSX(device) {
		...
		const result = device.abilities.map((abi) => {
			...
			let info = device
				? `stationid=${device.stationId}&&deviceid=${device.id}&&tasktype=${abi.name}`
				: "";
			strExecute += info;
			return (
				<Link
					...
					to={strExecute}
					target='_blank'
					onClick={(e) => linkToTaskClick(device, e)}
				>
					<div className='single_task_small'>
						<div
							className='single_task_img_small'
							style={{ background: "url(" + img + ") no-repeat center" }}
						></div>
						<div className='task_name_small'>
							{abi.name === "FIXFQ" ? "FIXED" : abi.name}
						</div>
					</div>
				</Link>
			);
		...
		return (
			<Collapse defaultActiveKey={["0"]} accordion>
				{station.devices.map((dev, index) => {
					return (
						<Panel
							header={dev.name + "---" + getDevStatusDes(dev)}
							key={index}
						>
	...
	return (
		<div className='realtime_stationinfo_box_container'>
			...
			<div className='realtime_device_task'>{getDeviceJSX()}</div>
		</div>
	);
};
