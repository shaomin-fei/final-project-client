...
function Reducer(preState, { type, data }) {
	switch (type) {
		case "stationTreeSelcChanged":
			return {
				...preState,
				signalInfo: { ...preState.signalInfo, station: data },
			};
		case "inputFreq":
			return {
				...preState,
				signalInfo: { ...preState.signalInfo, freq: data },
			};
		case "inputDescription": {
			return {
				...preState,
				signalInfo: { ...preState.signalInfo, description: data },
			};
		}
		case "inputLon": {
			return {
				...preState,
				signalInfo: { ...preState.signalInfo, Lon: data },
			};
		}
		case "inputLat": {
			return {
				...preState,
				signalInfo: { ...preState.signalInfo, Lat: data },
			};
		}
		case "updateSignal": {
...
const SignalOperation = function (props) {
	...
	function saveSignalInfo(cmd, signalInfo) {
		...
		if (
			signalInfo.station.length === 1 &&
			signalInfo.station[0] === stationTrees.name
		) {
			...
			<div className='content'>
						...
						<tr>
							...
							<td>
								<DatePicker
									...
									defaultValue={moment(
										state.signalInfo
											? state.signalInfo.findTime
											: Utils.dateFormat(dateFormat, new Date()),
										dateFormat
									)}
									format={dateFormat}
								/>
				...
				<div className='save_button'>
					<Button
						...
						onClick={() => saveSignalInfo(cmd, state.signalInfo)}
					>
						Save
					</Button>
				</div>
