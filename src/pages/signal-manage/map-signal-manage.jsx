...
const MapControls = [
	{
		...
		component: (props = {}) => {
			return <QueryByDateFC key={props.id} {...props} />;
		},
	},
	{
		element: null,
		...
		component: (props = {}) => {
			return <WarningLegendFC key={props.id} {...props} />;
		},
	},
	{
		element: null,
		...
		component: (props = {}) => {
			return <SignalFormFC key={props.id} {...props} />;
		},
	},
];
export default class MapSignalManage extends MapWithStationStatus {
	...
	deleteSignal = async (signal) => {
		try {
			const response = await Axios.delete(
				APIConfigEnum.deleteSingnalInfo,
				{
					params: {
						key: signal.key,
					},
			...
			if (data.success) {
				...
				this.querySignalByDate(
					queryCondition.startTime,
					queryCondition.stopTime
				);
			} else {
				message.warn(
					"delete fail " + data.errorInfo ? data.errorInfo : ""
				);
			}
		} catch (e) {
	...
	saveSignalInfoCallback = async (cmd, signalInfo, resultCallback) => {
		...
		try {
			...
			if (data.success) {
				...
				resultCallback(
					false,
					"Operation Fail " + data.errorInfo ? data.errorInfo : ""
				);
			}
		} catch (e) {
	...
	mapMouseClick = (evt) => {
		...
		if (feature) {
			...
			this.clearSignalLine();
			const featureLonLat = transform(
				evt.coordinate,
				"EPSG:3857",
				"EPSG:4326"
			);
			const featurePosition = new LonLat(
				featureLonLat[0],
				featureLonLat[1]
			);
	...
	connectSignalAndStation = (featurePosition, stasPosition) => {
		stasPosition.forEach((sta) => {
			const lay = this.centerMap.addLine(
				featurePosition,
				sta,
				3,
				"#2A3B54"
	...
	drawSignalsOnMap = (signals) => {
		...
		signals.forEach((signal) => {
			...
			this.signalLayer = this.centerMap.addCircle(
				point,
				this.signalLayer,
				signal,
				8000,
	...
	addAllControls() {
		const controls = [];
		controls.push(
			new QueryByDateControl({ element: MapControls[0].element })
		);
		controls.push(
			new WarningLegendControl({ element: MapControls[1].element })
		);
		controls.push(
			new SignalFormControl({ element: MapControls[2].element })
		);
			...
			<>
				...
				>
					...
					{controls.map((control) => {
						return control.component(control.props);
				...
				{this.state.showSignalInfo ? (
					<SignalInfoOnMap
						left={this.mouseLeft}
						top={this.mouseTop}
						// @ts-ignore
						signalInfo={this.state.mouseOverSignal}
					/>
				) : null}
