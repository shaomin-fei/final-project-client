...
import SignalOperation from "./signal-operation";
...
const SignalFormFC = function (props) {
			...
			case "addSignal": {
				return {
					...
					SignalOperationDlg: {
						...
						props: {
							cmd: "add",
							closeCallback: action.data.closeCallback,
				...
				return {
					...
					SignalOperationDlg: {
						...
						props: {
							cmd: "update",
							closeCallback: action.data.closeCallback,
							signalInfo: action.data.signalInfo,
							centerInfo: preState.centerInfo,
							saveSignalInfoCallback: props.saveSignalInfoCallback,
						},
					},
  ...
  console.log(props);
	...
	useEffect(() => {
		dispatch({
			type: "treeSelectRowChanged",
			data: selectedSignalOnMap ? [selectedSignalOnMap.key] : [],
		});
	}, [selectedSignalOnMap]);
	//   useEffect(()=>{
	...
	function modifySignal(signal) {
		dispatch({
			type: "updateSignalClick",
			data: { closeCallback: dlgSignalClose, signalInfo: signal },
		});
	...
	function onSelectedRowKeysChange(selectedRowKeys, selectedRows) {
		...
		dispatch({
			type: "treeSelectRowChanged",
			data: selectedRowKeys.length > 0 ? [selectedRowKeys[0]] : [],
		});
	}
		...
		>
			<div className='signal_form_header'>
				...
				<span className='zoom_in_form_button'>
					{!state.formBigger ? (
						<ExportOutlined
							onClick={(e) => changeFormSizeClick("zoomin")}
						/>
					) : (
						<ImportOutlined
							onClick={(e) => changeFormSizeClick("zoomout")}
						/>
					)}
				</span>
			...
			{state.SignalOperationDlg.show ? (
				<SignalOperation {...state.SignalOperationDlg.props} />
			) : null}
		</div>
	);
};
