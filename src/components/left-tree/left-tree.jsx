...
class LeftTree extends Component {
	...
	onChange = (e) => {
		...
		let expandedKeys = [];
		this.state.treeData &&
			this.state.treeData.forEach((tree) => {
	...
	expandOrUnexpandNode = (info) => {
		...
		this.setState({
			...
			autoExpandParent: index === -1 ? true : false,
		});
	};
	...
	render() {
		...
		if (taskInfo && this.firstRenderWhenTriggered) {
			...
			tasks = currentDevice
				? currentDevice.abilities.map((ability) => {
						return ability.name;
				  })
				: [];
		}
