...
export default class BaseMap extends Component {
	...
	loadMap(initInfo, loadedCallBack) {
		...
		this.map = new Map({
			...
			view: new View({
				center: fromLonLat([
					initInfo.centerPosition.lon,
					initInfo.centerPosition.lat,
				]),
		...
		initInfo.mousePositionTargetId &&
			this.loadMousePosition(initInfo.mousePositionTargetId);
		initInfo.onMapClick &&
			this.map.addEventListener("click", initInfo.onMapClick);
		//this.updateSize();
	...
	panMap(newScreenOffsetX, newScreenOffsetY) {
		const centerPix = this.map.getPixelFromCoordinate(
	...
	componentWillUnmount() {
		...
		this.iniInfo &&
			this.iniInfo.onMapClick &&
			this.map.removeEventListener("click", this.iniInfo.onMapClick);
	}
